# 3D Gaussian Splatting — Windows Setup & Learning Reference
...
This repository serves as my hands-on workspace for understanding and experimenting with 3D Gaussian Splatting — a real-time radiance field rendering technique that reconstructs photorealistic 3D scenes from multi-view images.
...
> **Note on 8 GB VRAM:** The default training config targets 24 GB GPUs. To train on 8 GB, I adjust densification thresholds (`--densify_grad_threshold`), reduce `--densify_until_iter`, and use `--data_device cpu` to offload source images from VRAM. Quality trades off against memory — documenting these experiments is part of this learning journey.
...
# Prepare your dataset (requires COLMAP on PATH)
python convert.py -s <path/to/your/images> --resize

# Train
python train.py -s <path/to/prepared/dataset>
...
- **Original Paper:** Kerbl, B., Kopanas, G., Leimkühler, T., & Drettakis, G. (2023). *3D Gaussian Splatting for Real-Time Radiance Field Rendering.* ACM Transactions on Graphics (SIGGRAPH 2023). [[Paper]](https://repo-sam.inria.fr/fungraph/3d-gaussian-splatting/3d_gaussian_splatting_high.pdf) [[Project Page]](https://repo-sam.inria.fr/fungraph/3d-gaussian-splatting/)
- **Original Repo:** [graphdeco-inria/gaussian-splatting](https://github.com/graphdeco-inria/gaussian-splatting)
- **Windows Tutorial Fork:** [jonstephens85/gaussian-splatting-Windows](https://github.com/jonstephens85/gaussian-splatting-Windows) — detailed Windows installation walkthrough that this repo is forked from
- **Walkthrough Video:** [YouTube — Jon Stephens](https://youtu.be/UXtuigy_wYc)

---

## License

This project inherits the license from the original Gaussian Splatting repository. See [LICENSE.md](LICENSE.md) for details. The original work was funded by ERC Advanced grant FUNGRAPH No 788065.
